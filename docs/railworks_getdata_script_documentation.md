...
## Descripción General
El script `Railworks_GetData_Script.lua` es un plugin para Train Simulator Classic (RailWorks) que recopila datos de telemetría en tiempo real de la locomotora y los escribe en un archivo `GetData.txt`. Este archivo es utilizado por aplicaciones externas (como el autopilot de TrainSimulatorAutopilot) para monitorear y controlar el tren.
...
1. **`getdata()`**: Función principal llamada periódicamente. Verifica si el jugador está conduciendo y recopila datos.
2. **`GetSpeedInfo()`**: Obtiene información de velocidad y tipo de velocímetro (MPH/KPH).
...
## Configuración
- **`delay`**: Número de iteraciones antes de actualizar datos (por defecto 1 para actualizaciones rápidas).
- **`BASE_DIR`**: Directorio base para archivos (plugins de RailWorks).
...
El script alimenta datos al backend Python (`tsc_integration.py`), que parsea `GetData.txt` y envía telemetría al dashboard web. Asegúrate de que el simulador se ejecute como administrador para permisos de escritura.
