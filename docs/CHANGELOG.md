...
adhiere al [Versionado Semántico](https://semver.org/spec/v2.0.0.html).
...
---------------- | ------------------ | -------- | -------------- | | Tiempo de
inicio | ~30s (con errores) | ~5s      | 83% más rápido | | Tasa de éxito    |
0%                 | 100%     | 100%           | | Compatibilidad   | Limitada
| Completa | Total          |
...
- **Actions Version**: Actualizado a `upload-artifact@v4`,
...
- **Documentación**: Actualizada en "data-received-from-railworks.md" como
"[IMPLEMENTADO]"
...
- **Compatibilidad**: Variable `FuelLevel` mantenida disponible para futuras
  expansiones o otros simuladores. Se agregó el script
  `scripts/cleanup_persisted_fuel.py` para limpiar datos históricos de
  `FuelLevel`.
...
`data-received-from-railworks.md`
...
- ✅ **Documentación Exhaustiva**: Referencia completa de API, guía del
framework de pruebas y documentación del proyecto
...
- 🐛 **Modal del Botón de Configuración**: Convertido el panel de configuración
a modal de Bootstrap para mejor UX. Anteriormente, el panel de configuración al
